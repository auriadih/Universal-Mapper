...
-- Snomed II terms from OYS

-- T_SNOMED
drop materialized view if exists snomed.mv_oys_t_snomed;

create materialized view snomed.mv_oys_t_snomed as
with cte as (
select 
t_snomed, elin, min(alkuhetki) as alkuhetki, max(loppuhetki) as loppuhetki, sum(lkm) as lkm
from snomed.oys_snomed2_elimet
...
alkuhetki as min_ts,
case when loppuhetki != '2017-12-29 00:00:00' then loppuhetki else null end as max_ts

from cte;


-- M_SNOMED
drop materialized view if exists snomed.mv_oys_m_snomed;

create materialized view snomed.mv_oys_m_snomed as
with cte as (
select 
m_snomed, diagnoosi, min(alkuhetki) as alkuhetki, max(loppuhetki) as loppuhetki, sum(lkm) as lkm
from snomed.oys_snomed2_diagnoosit
...
from snomed.mv_oys_t_snomed on conflict do nothing;

insert into code_mapper.term (term_text)
select distinct diagnoosi
from snomed.mv_oys_m_snomed on conflict do nothing;
...
select distinct t_snomed_original,
	c.code_id,
		...
		where d.dialect_name = 'PPSHP dialect' and c.name = 'Snomed II')
from snomed.mv_oys_t_snomed as s
...
select distinct m_snomed_original,
		...
		(select dialect_id from code_mapper.dialect as d
			inner join code_mapper.code_system as c on d.code_system_id = c.code_system_id
			where d.dialect_name = 'PPSHP dialect' and c.name = 'Snomed II')
from snomed.mv_oys_m_snomed as s
...
from concepts as s
inner join code_mapper.organisation  as o on o.name = 'PPSHP'
inner join code_mapper.dialect as d on d.dialect_name = 'PPSHP dialect'
...
inner join code_mapper.organisation as o on co.organisation_id = o.organisation_id and o.name = 'PPSHP'
inner join code_mapper.dialect as d on co.dialect_id = d.dialect_id and d.dialect_name = 'PPSHP dialect';
