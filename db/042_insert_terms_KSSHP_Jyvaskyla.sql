...
-- Snomed II terms from KSSHP

-- T_SNOMED
drop materialized view if exists snomed.mv_ksbp_t_snomed;
create materialized view snomed.mv_ksbp_t_snomed as
select
   ...
   FROM snomed.ksbp_snomed2_elimet as e;
...
-- M_SNOMED
drop materialized view if exists snomed.mv_ksbp_m_snomed;
create materialized view snomed.mv_ksbp_m_snomed as
select
   ...
   FROM snomed.ksbp_snomed2_diagnoosit as e;
...
from snomed.mv_ksbp_t_snomed on conflict do nothing;

insert into code_mapper.term (term_text)
select distinct diagnoosi
from snomed.mv_ksbp_m_snomed on conflict do nothing;
...
select distinct t_snomed_original,
	c.code_id,
		...
		where d.dialect_name = 'KSSHP dialect' and c.name = 'Snomed II')
from snomed.mv_ksbp_t_snomed as s
...
select distinct m_snomed_original,
		...
		(select dialect_id from code_mapper.dialect as d
			inner join code_mapper.code_system as c on d.code_system_id = c.code_system_id
			where d.dialect_name = 'KSSHP dialect' and c.name = 'Snomed II')
from snomed.mv_ksbp_m_snomed as s
...
from concepts as s
inner join code_mapper.organisation  as o on o.name = 'KSSHP'
inner join code_mapper.dialect as d on d.dialect_name = 'KSSHP dialect'
...
inner join code_mapper.organisation as o on co.organisation_id = o.organisation_id and o.name = 'KSSHP'
inner join code_mapper.dialect as d on co.dialect_id = d.dialect_id and d.dialect_name = 'KSSHP dialect';
